 ...
 * for free. If someone ever lands a hundred thousand rows here, revisit it then.
 */
...
function build() {
  ...
  // Newest first is the baseline order every sort refines.
  index.sort((a, b) => b.row.ts - a.row.ts)
  return index
}
...
function passesFilters(entry, f) {
  ...
  for (const has of f.has) {
    ...
    if (has === 'image' || has === 'images' || has === 'photo') {
      if (!row.media.some((m) => m.type === 'photo')) return false
    }
    if (has === 'video' && !row.media.some((m) => m.type !== 'photo')) return false
    if (has === 'link' || has === 'links') {
      if (!row.links.length) return false
    }
  }
...
function scoreTerm(entry, term) {
  let score = 0
  const boundary = new RegExp(`(^|[^a-z0-9])${term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'i')
...
export function search({ query = '', sort = 'relevance', limit = 300 } = {}) {
  ...
  return {
    total: hits.length,
    terms,
    results: hits.slice(0, limit).map((h) => h.row),
  }
}
