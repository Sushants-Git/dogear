...
import { copyText, openUrl } from '../lib/browser.js'
...
function Media({ media }) {
  // Loaded straight from X's CDN, and plenty of it is gone by the time you read this.
  // A dead image should leave nothing behind rather than a broken-image placeholder,
  // so each one drops itself and the frame disappears once they all have.
  const [dead, setDead] = useState(() => new Set())
  const alive = media.filter((_, i) => !dead.has(i))
  if (!alive.length) return null

  const kill = (i) => setDead((prev) => new Set(prev).add(i))

  return (
    <div className={`media media-${Math.min(alive.length, 4)}`}>
      {media.map((m, i) =>
        dead.has(i) ? null : m.type === 'photo' ? (
          <img key={i} src={m.url} alt="" loading="lazy" onError={() => kill(i)} />
        ) : (
          <video key={i} src={m.url} controls preload="metadata" onError={() => kill(i)} />
        )
      )}
    </div>
  )
...
function PostInner({ row, terms, active, previews, onSelect, onRemove, cardRef }) {
    ...
    >
      <header className="card-head">
        <button
          type="button"
          className="card-who"
          onClick={() => openUrl(`https://x.com/${row.author}`)}
      ...
      {row.links.map((link) => (
        <LinkCard key={link} url={link} previews={previews} />
      ))}

      {row.quoted_url && (
        <button type="button" className="card-quote" onClick={() => openUrl(row.quoted_url)}>
          <QuoteIcon size={13} />
      ...
      <footer className="card-foot">
        ...
        <span className="card-tools">
          <button type="button" className="tool" title="Copy link — ⌘⇧C" onClick={() => copyText(row.url)}>
            <CopyIcon size={14} />
          </button>
          <button
            type="button"
