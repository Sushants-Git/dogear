/* Open Graph parsing, as used by the service worker for link previews. */
import test from 'node:test'
import assert from 'node:assert/strict'
import { parseMeta } from '../static/og.js'
...
test('reads the Open Graph tags', () => {
  const meta = parseMeta(`
    ...
    <meta property="og:site_name" content="Example">`, 'https://example.com/post')
...
test('falls back to twitter: tags, then to <title>', () => {
  const twitter = parseMeta('<meta name="twitter:title" content="Via Twitter"><meta name="twitter:image" content="https://x/y.jpg">', 'https://example.com')
...
test('resolves a relative image against the page it came from', () => {
  assert.equal(
    parseMeta('<meta property="og:image" content="/img/hero.png">', 'https://example.com/a/b').image,
...
test('decodes entities, named and numeric', () => {
  assert.equal(
    parseMeta('<meta property="og:title" content="Tips &amp; tricks &#39;24 &quot;quoted&quot; &#x2014;">', 'https://e.com').title,
    'Tips & tricks \'24 "quoted" —'
  )
})

test('handles single quotes, odd spacing and attribute order', () => {
  const meta = parseMeta("<meta content='Odd' property='og:title' ><meta   property = \"og:description\"   content = \"Spaced\">", 'https://e.com')
...
test('the first tag wins when a page states one twice', () => {
  assert.equal(
    parseMeta('<meta property="og:title" content="First"><meta property="og:title" content="Second">', 'https://e.com').title,
    'First'
  )
})

test('a page with nothing useful returns nulls rather than throwing', () => {
  assert.deepEqual(parseMeta('<html><body><p>hi</p><meta broken</body></html>', 'https://e.com'),
    { title: null, description: null, site: null, image: null })
})
